...
class AutoCrackerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("纯数字字典生成与破译机 (可跳转进度版)")
        self.root.geometry("580x820")  # 适当增加高度容纳新选项
        self.root.attributes("-topmost", True)
        
        self.config = self.load_config()
        self.passwords = []
        self.total_passwords = 0
        self.current_index = 0  # 核心改动：全局进度指针
    ...
    def setup_ui(self):
        ...
        self.btn_del_config.grid(row=2, column=0, columnspan=4, pady=10)
    ...
    def jump_to_index(self):
        ...
        if not self.passwords:
            messagebox.showinfo("提示", "请先点击【1. 生成字典】！")
            return
        try:
            idx = int(self.entry_jump.get())
            if 1 <= idx <= self.total_passwords:
                self.current_index = idx - 1  # 内部指针从0开始
    ...
    def brute_force_thread(self):
        ...
        # 核心改动：不再使用死板的 for 循环，改为灵活的 while 全局指针控制
        while self.current_index < self.total_passwords:
            ...
            if self.is_paused: continue  # 如果中途暂停，跳出重新等待，指针不加一
